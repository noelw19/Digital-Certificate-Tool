...
export function ValidateCert({setModal}) {
    ...
    function decodePEMPromise(pem) {
        return new Promise((resolve) => {
            const res = window.validateCertificate(pem);
            resolve(res);
        });
    }

    return (
        <Page title="Validate Certificate">
            <div>
            <p className="text-red-500 font-bold">Validation logic incomplete</p>
            <p className="text-red-500">Only checks issue, expiry and whether CA is trusted by your system</p>
            </div>
            ...
            {(created && !loading) && <div className="w-[75%] p-4 flex justify-center">
                {/* {created.type === "CERTIFICATE" ? certViewer(created)} */}
                {certViewer(created)}
...
function quickSpan(check, data) {
    return (<span className={`${check ? "text-red-500": "text-green-500"}`}>{data}</span>)
}
    ...
    return (
        <div className="w-[70%] text-left">
            <div className="my-4 pb-4 border-b-2">
                <p className="text-xl font-bold text-center pb-4">Validation data:</p>

                <p>Issue: {quickSpan(data["Check-Issue"], data["Check-Issue"] ? "Certificate not officially issued" : "Valid")}</p>
                <p>Expiry: {quickSpan(data["Check-Expiry"], data["Check-Expiry"] ? "Certificate expired" : "Valid")}</p>
                <p>CA: {quickSpan(data["Check-CA"], data["Check-CA"] ? "Unknown CA" : "Valid")}</p>
            </div>
...
export default function PemDecoder({ setPem }) {
    ...
    return (
        <div className="w-full h-[100%] mx-auto mb-4 p-2 border rounded-lg shadow-md overflow-none">
            ...
            <div className="mb-4">
                <input type="file" accept=".pem,.crt" onChange={handleFileInputChange} />
            </div>
